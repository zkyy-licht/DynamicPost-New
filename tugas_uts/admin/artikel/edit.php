...
// Ambil data artikel yang akan diedit, periksa kepemilikan untuk author
...
// Author hanya bisa edit artikel miliknya
...
// Ambil tag yang sudah dipilih untuk artikel ini
$stmt_artikel_tag = $pdo->prepare("SELECT tag_id FROM artikel_tag WHERE artikel_id = ?");
$stmt_artikel_tag->execute([$id]);
$selected_tags = $stmt_artikel_tag->fetchAll(PDO::FETCH_COLUMN);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Update artikel
            $sql = "UPDATE artikel SET judul=?, slug=?, konten=?, gambar=?, kategori_id=?, status=? WHERE id=?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$judul, $slug, $konten, $gambar, $kategori_id, $status, $id]);
            
            // Update relasi tag: hapus semua dulu, lalu insert ulang
            $stmt_del = $pdo->prepare("DELETE FROM artikel_tag WHERE artikel_id = ?");
            $stmt_del->execute([$id]);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Artikel - DynamicPost Admin</title>
...
<div class="container mt-4">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label>Konten *</label>
            <textarea name="konten" id="editor" class="form-control" rows="10"><?= htmlspecialchars($artikel['konten']) ?></textarea>
        </div>
        <div class="form-group">
            ...
            <select name="kategori_id" class="form-control" required>
                ...
                <?php foreach ($kategori_list as $kat): ?>
                    <option value="<?= $kat['id'] ?>" <?= ($kat['id'] == $artikel['kategori_id']) ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <?php if ($artikel['gambar']): ?>
                <img src="../../uploads/<?= $artikel['gambar'] ?>" style="max-height: 150px;">
                <br>
            <?php endif; ?>
